...
from .measurement_models import ScalarNoiseCovariance
...
class PseudoinverseDiffusionBridgeModel(torch.nn.Module):
    def __init__(self,  
        ...
        super(PseudoinverseDiffusionBridgeModel, self).__init__()
...
class ImageToImageSchrodingerBridgeModel(torch.nn.Module):
    def __init__(self,  
                measurement_likelihood,
                denoiser_network,
                i2sb_variance):

        super(ImageToImageSchrodingerBridgeModel, self).__init__()
        assert isinstance(measurement_likelihood, LinearSystemPlusGaussianNoise)

        i2sb_noise_covariance = ScalarNoiseCovariance(i2sb_variance)

        self.measurement_likelihood = measurement_likelihood    
        self.i2sb_noise_covariance = i2sb_noise_covariance
        self.denoiser_network = denoiser_network
    ...
    def sample_z_t_given_x_0_and_white_noise(self, x_0, t, white_noise):
        while len(t.shape) < len(x_0.shape):
            t = t.unsqueeze(-1)
        # Sample y using the measurement likelihood and apply the pseudoinverse reconstruction
        y = self.measurement_likelihood.sample(x_0)
        pinvA_y = self.measurement_likelihood.mean_system_response.pinvA(y)
        
        range_space_noise = self.measurement_likelihood.noise_covariance.sqrtSigma(white_noise)
        range_space_noise = self.measurement_likelihood.mean_system_response.pinvA(range_space_noise)
        range_space_noise = (1 - t) * range_space_noise

        i2sb_white_noise = torch.randn(x_0.shape, dtype=x_0.dtype, device=x_0.device)
        i2sb_white_noise = self.i2sb_noise_covariance.sqrtSigma(i2sb_white_noise)
        i2sb_white_noise = 2*(t - t**2)**0.5 * i2sb_white_noise

        return (1-t)*x_0 + t*pinvA_y + i2sb_white_noise
    ...
    #     if type(dt) is torch.Tensor:
    #         dt = float(dt.detach().cpu().numpy())
    #     i2sb_white_noise = torch.randn(z_t.shape, dtype=z_t.dtype, device=z_t.device)
    #     i2sb_white_noise = self.i2sb_noise_covariance.sqrtSigma(i2sb_white_noise)
    #     i2sb_white_noise = (dt - dt**2)**0.5 * i2sb_white_noise
    #     return z_t + i2sb_white_noise
    ...
    def sample_z_t_minus_dt_given_z_t_and_white_noise(self, z_t, t, dt, white_noise):
        ...
        i2sb_white_noise = torch.randn(z_t.shape, dtype=z_t.dtype, device=z_t.device)
        i2sb_white_noise = self.i2sb_noise_covariance.sqrtSigma(i2sb_white_noise)
        i2sb_white_noise = 2* np.sqrt(dt) * i2sb_white_noise
        return z_t + drift + i2sb_white_noise
...
class ConditionalDiffusionModel(torch.nn.Module):
    ...
    def sample_z_t_given_x_0(self, x_0, t):
        while len(t.shape) < len(x_0.shape):
            t = t.unsqueeze(-1)
        noise = torch.randn_like(x_0, device=x_0.device)
        return x_0 + torch.sqrt(t) * self.sigma_max * noise

    def sample_z_t_minus_dt_given_z_t(self, z_t, x_tilde, t, dt):
        while len(t.shape) < len(z_t.shape):
            t = t.unsqueeze(-1)
        if isinstance(dt, torch.Tensor):
            dt = float(dt.detach().cpu().numpy())
        mean_estimate = self.denoiser_network(z_t, x_tilde, t)
        drift = dt * (1 / t) * (mean_estimate - z_t)
        noise = torch.randn_like(z_t, device=z_t.device)
        noise_term = np.sqrt(dt) * self.sigma_max * noise
        return z_t + drift + noise_term
    ...
    def train(self, dataset, batch_size, num_epochs, learning_rate, warmup_steps=0, batches_per_epoch=100, loss_fn=None):
        ...
        for epoch in range(num_epochs):
            for iBatch, [batch] in enumerate(data_loader):
                ...
                t = torch.rand((x_0.shape[0], 1), device=x_0.device)
                y = self.measurement_likelihood.sample(x_0)
                x_tilde = self.measurement_likelihood.mean_system_response.pinvA(y)
                z_t = self.sample_z_t_given_x_0(x_0, t)
                x_0_hat = self.denoiser_network(z_t, x_tilde, t)
                loss = loss_fn(x_0, x_0_hat, t)
                loss.backward()
                optimizer.step()
                scheduler.step()
    ...
    def sample(self, y, time_steps=None, num_time_steps=None, return_all_timesteps=False, verbose=False):
        ...
        assert isinstance(time_steps, torch.Tensor)
        assert time_steps.shape[0] > 1, 'Time steps must have at least 2 elements'
        assert time_steps[0] == 1.0, 'Initial time must be 1.0'
        for i in range(1, time_steps.shape[0]):
            assert time_steps[i] < time_steps[i - 1], 'Time steps must be decreasing'
        ...
        pinvA_y = self.measurement_likelihood.mean_system_response.pinvA(y)
        noise = torch.randn_like(pinvA_y, device=pinvA_y.device)
        z_T = self.sigma_max * noise
        ...
        for i in range(num_timesteps):
            ...
            if verbose:
                print('Timestep: %d / %d, from time %f to time %f' % (i + 1, num_timesteps, t[0, 0].item(), time_steps[i + 1].item()))
            with torch.no_grad():
                z_t = self.sample_z_t_minus_dt_given_z_t(z_t, pinvA_y, t, dt)
            t[:,0] = time_steps[i+1]
            if return_all_timesteps:
                z_t_all[i+1] = z_t
